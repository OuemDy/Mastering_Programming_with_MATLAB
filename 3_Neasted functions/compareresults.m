function pass = compareresults(x,y)

% MATLAB EXERCISE
% Write a function that compare two different functions' results.
% for instance: compareresults(max([1,2,3]),min([3,4,5]))
% By Olivia Marin

if isequal(x,y)
    fprintf('Both results are the same: %d and %d\n',x,y)
    pass = true;
else
    fprintf('Both results are different: %d and %d\n',x,y)
    pass = false;
end
